...
class GanFnsAndTypes(object):
  ...
  state_disc_optimizer_fn = attr.ib()
  state_gen_optimizer_fn = attr.ib()

  # Decentralization status of each component of the GAN
  gen_status = attr.ib(type=str)
  disc_status = attr.ib(type=str)
  ...
  state_gen_opt_weights_type = attr.ib(init=False)
  state_disc_opt_weights_type = attr.ib(init=False)
  ...
  def __attrs_post_init__(self):
    ...
    gan_training_tf_fns.initialize_optimizer_vars(self._generator, self._state_gen_opt)
    gan_training_tf_fns.initialize_optimizer_vars(self._discriminator, self._state_disc_opt)
    ...
    def vars_to_type(var_struct):
      ...
      return tf.nest.map_structure(
          lambda v: tf.TensorSpec.from_tensor(tf.cast(v.read_value(), tf.float32)), var_struct)
    def vars_to_type_counter(var_struct):
      # TODO(b/131681951): read_value() shouldn't be needed
      return tf.nest.map_structure(
          lambda v: tf.TensorSpec.from_tensor(v), var_struct)

    self.discriminator_weights_type = vars_to_type(gan_training_tf_fns._weights(self._discriminator))
    self.generator_weights_type = vars_to_type(gan_training_tf_fns._weights(self._generator))
    self.state_gen_opt_weights_type = vars_to_type(self._state_gen_opt.variables())
    self.state_disc_opt_weights_type = vars_to_type(self._state_disc_opt.variables())
...
def build_server_initial_state_comp(gan: GanFnsAndTypes):
  ...
  def server_initial_state():
    ...
    return gan_training_tf_fns.server_initial_state(generator, discriminator,gen_opt, disc_opt,
                                                    dp_averaging_state)
...
def build_client_computation(gan: GanFnsAndTypes):
  ...
  def client_computation(gen_inputs, real_data, from_server):
    ...
    scheduler = tf.keras.optimizers.schedules.PiecewiseConstantDecay([1000], [0.001, 0.0005])
    generator = gan.generator_model_fn()
    state_gen_optimizer = gan.state_gen_optimizer_fn(scheduler.__call__(steps))
    gan_training_tf_fns.initialize_optimizer_vars(generator, state_gen_optimizer)
    discriminator = gan.discriminator_model_fn()
    state_disc_optimizer = gan.state_disc_optimizer_fn(0.0002)
    gan_training_tf_fns.initialize_optimizer_vars(discriminator, state_disc_optimizer)
...
def build_gan_training_process(gan: GanFnsAndTypes):
  ...
  def run_one_round(server_state, server_gen_inputs, client_gen_inputs,
    ...
    from_server = gan_training_tf_fns.FromServer(
        ...
        state_gen_optimizer_weights=server_state.state_gen_optimizer_weights,
        state_disc_optimizer_weights=server_state.state_disc_optimizer_weights,
